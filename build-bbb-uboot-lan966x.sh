...
# => setenv pcb 'pcb134'
# => setenv bootcmd 'dhcp <TFTP_IP>:<path>/armv7_lan966x_bbb.itb; run ramboot'
# => saveenv
#
# The armv7_lan966x_bbb is build in the next section:
else
    echo "bbb.img alreadfy exist so skipping that part"
fi


# (2.1) --- Get MESA and prepare a mesa-demo build for LAN966X running on Beaglebone.
#
...
cd mesa-v2024.03
.cmake/create_cmake_project.rb arm my-lan966x_bbb
cd my-lan966x_bbb
./cmake .. -DIMG_armv7_lan966x_bbb_fit=ON
